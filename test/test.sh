...
_mainScript_() {
  ...
  testline="${purple}testing and testing [33magain"
  #testline="$(echo "$testline" | sed -E 's/\[[0-9;]+m//g')"
  testline="$(echo "$testline" | gsed -E 's/\x1b\[(([0-9]{1,2})(;[0-9]{1,3}){0,2})?[mGK]//g')"
  echo "$testline"
  echo "$testline" > $HOME/Desktop/LogLOGLOGLOGLOGLOGLOG.txt
