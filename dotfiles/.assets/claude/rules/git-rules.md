...
### Troubleshooting: git hooks not found in worktrees

If git cannot find git hook scripts in a worktree (not tool failures — those are typically `.venv` issues), manually point the worktree to the shared hooks directory:
